...
int pidD(double wD)
{
  ...
  if(errorD>=0.30|| errorD<(-0.30))
  {
    ...
    if(lastErrorD<0 && errorD>0)cumErrorD=errorD;
    if(cumErrorD>20|| cumErrorD<-20) cumErrorD=0;                         //se resetea el error acumulativo
    rateErrorD = (errorD - lastErrorD) / SAMPLINGTIME*1000;         // calcular la derivada del error
    outputD =static_cast<int> (round(0.1*errorD +0.08*cumErrorD + 0*rateErrorD ));     // calcular la salida del PID     0.0025*cumErrorD
...
int pidI(double wI)
{
  ...
  if(errorI>=0.3 || errorI<(-0.3))
  {
    ...
    if(cumErrorI>20||cumErrorI<-20) cumErrorI=0;     //se resetea el error acumulativo 
    rateErrorI = (errorI - lastErrorI) /(SAMPLINGTIME*1000);         // calcular la derivada del error
    
    outputI = static_cast<int> (round(0.1*errorI  + 0.078*cumErrorI + 0*rateErrorI));     // calcular la salida del PID 
    lastErrorI = errorI; 
...
void feedForwardD()
{
    ...
    {
      PWM_D=round((setpointWD-0.25495)/0.0669);
      if(PWM_D<94){
        PWM_D=94;
...
{   if(setpointWI==0)
    {
      ...
      PWM_I=round((setpointWI-0.682598)/0.063015);
      if(PWM_I<94)
      {
        PWM_I=94;
        //strcpy((char*)operation_send.data,"mensaje recibido, alerta pwm en discontinuidad");
